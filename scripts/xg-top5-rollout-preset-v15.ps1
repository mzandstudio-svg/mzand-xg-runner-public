...
function FindLargeSubmenu($root,[int]$processId){
  $all=$root.FindAll([System.Windows.Automation.TreeScope]::Descendants,[System.Windows.Automation.Condition]::TrueCondition);$out=@()
  foreach($e in $all){try{$r=$e.Current.BoundingRectangle;if($e.Current.ProcessId-eq$processId -and $e.Current.ClassName-eq'#32768' -and $r.Width-gt300 -and $r.Height-gt300){$out+=,$e}}catch{}}
  return @($out)
}
...
'XG Top5 Rollout v15 Explicit Preset Confirm'|Out-File $report
...
$mainWin=$null
foreach($e in $tops){try{if($e.Current.ProcessId-eq$xg.Id -and $e.Current.ClassName-eq'TMainX'){$mainWin=$e;break}}catch{}}
if($null-eq$mainWin){throw 'TMainX window not found'}
$mr=$mainWin.Current.BoundingRectangle
"MAIN_RECT: $($mr.X),$($mr.Y),$($mr.Width),$($mr.Height)"|Out-File $report -Append

$subs=FindLargeSubmenu $root $xg.Id
if($subs.Count-ne1){throw "Expected initial Rollout submenu once, got $($subs.Count)"}
...
"PRESET_ITEM_ID: Item 18"|Out-File $report -Append;"PRESET_Y_DELTA: $pdelta"|Out-File $report -Append
if($pdelta-lt55 -or $pdelta-gt70){throw "Preset geometry mismatch $pdelta"}
LeftClick ([int]($pr.X+$pr.Width/2)) ([int]($pr.Y+$pr.Height/2))
Post 'xg-top5-v15/preset-selected' 'success' 'Explicit preset Item 18 selected'
...
if($contexts.Count-ne1){Shot 'xg-top5-v15-context-mismatch';DumpUI 'xg-top5-v15-context-mismatch' $xg.Id;throw "Expected context popup once, got $($contexts.Count)"}
$cr=$contexts[0].Current.BoundingRectangle
[V15N]::SetCursorPos([int]($cr.X+$cr.Width/2),[int]($cr.Y+221))|Out-Null
Start-Sleep 1
$subs=FindLargeSubmenu $root $xg.Id
if($subs.Count-ne1){Shot 'xg-top5-v15-second-submenu-mismatch';DumpUI 'xg-top5-v15-second-submenu-mismatch' $xg.Id;throw "Expected reopened Rollout submenu once, got $($subs.Count)"}
$sub=$subs[0];$sr=$sub.Current.BoundingRectangle
$desc=$sub.FindAll([System.Windows.Automation.TreeScope]::Descendants,[System.Windows.Automation.Condition]::TrueCondition)
$start=@()
...
"START_ITEM_ID: Item 27"|Out-File $report -Append;"START_Y_DELTA: $sdelta"|Out-File $report -Append
if($sdelta-lt225 -or $sdelta-gt240){throw "Start geometry mismatch $sdelta"}
LeftClick ([int]($rr.X+$rr.Width/2)) ([int]($rr.Y+$rr.Height/2))
Post 'xg-top5-v15/start-clicked' 'success' 'Start clicked after explicit preset Item 18 selection'
Start-Sleep 1
Shot 'xg-top5-v15-prompt-1s';DumpUI 'xg-top5-v15-prompt-1s' $xg.Id
Start-Sleep 3
Shot 'xg-top5-v15-prompt-4s';DumpUI 'xg-top5-v15-prompt-4s' $xg.Id
Post 'xg-top5-v15/prompt-captured' 'success' 'Prompt captured after explicit preset selection'
Get-Content $report
