...
The v34 residual improved hard-position Top-1 but regressed overall Top-1.
v35 therefore learns the same train-only residual, selects both blend strength and
an inference-time base-confidence gate on tune only, refits on train+tune, then
opens sealed dev exactly once. No dev mining/model selection, pristine, or XG data.
"""
...
def main():
    ...
    trials=[]; best=None
    # Includes exact frozen-v33 fallback (beta=0), so tune cannot force residual use.
    ...
    out={'model':'mzand-gnu-v35','architecture':'V33_PLUS_CONFIDENCE_GATED_HARD_RESIDUAL','counts':counts,'selectedBeta':beta,'selectedGate':gate,'trainMining':stats,'finalMining':finalstats,'tuneTrials':trials,'dev':dev,'devUsedForModelSelection':False,'devRowsMined':0,'pristineDataUsed':False,'xgLabelsUsed':False}
