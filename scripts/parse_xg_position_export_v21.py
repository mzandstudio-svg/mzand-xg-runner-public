...
MATCH_SCORE_RE = re.compile(
    r"^Score is X:(\d+) O:(\d+) (\d+) pt\.\(s\) match\.\s*$", re.MULTILINE
)
UNLIMITED_SCORE_RE = re.compile(
    r"^Score is X:(\d+) O:(\d+)\.\s*Unlimited Game\s*$", re.MULTILINE
)
...
def analysis_method(source: str, note: str | None):
    """Classify the source itself, never historical Book footnotes as fresh rollout."""
    source_clean = source.strip()
    source_lower = source_clean.lower()
    note = note or ""

    # A Book row remains Book even when its historical provenance says it was rolled.
    # This is critical: historical opening-book rollouts are not evidence that the
    # current CI run completed a fresh rollout.
    if source_lower.startswith("book"):
        return "Book"
    if source_lower.startswith("rollout"):
        return "Rollout"
    if "xg roller++" in source_lower:
        return "XG Roller++"
...
def parse_score(text: str):
    match = MATCH_SCORE_RE.search(text)
    if match:
        return {
            "x": int(match.group(1)),
            "o": int(match.group(2)),
            "match_length": int(match.group(3)),
            "unlimited_game": False,
        }
    unlimited = UNLIMITED_SCORE_RE.search(text)
    if unlimited:
        return {
            "x": int(unlimited.group(1)),
            "o": int(unlimited.group(2)),
            "match_length": None,
            "unlimited_game": True,
        }
    raise ValueError("Required score header missing or unsupported")
...
def parse_export(text: str):
    ...
    if not cube_match or not roll_match:
        raise ValueError("Required cube/on-roll header missing")
    ...
    return {
        "schema": "mzand.xg.position-label.v2",
...
def main():
    parser = argparse.ArgumentParser(description="Parse eXtreme Gammon position clipboard export (v21)")
    ...
    print(f"unlimited_game={data['score']['unlimited_game']}")
