...
if($row.analysis_found -and $row.mentions_1ply){
  $scanOut=Join-Path $env:MZ_ALLNET_OUT ([string]$c.case_id)
  New-Item -ItemType Directory -Force -Path $scanOut|Out-Null
  & python $env:MZ_ALLNET_SCRIPT ([string]$xg.Id) $env:MZ_ALLNET_MODEL $scanOut
  if($LASTEXITCODE -ne 0){throw "allnet decoder exit $LASTEXITCODE"}
}
'@
$src=$src.Replace($needle,$inject)
$temp=Join-Path $env:RUNNER_TEMP 'CAPTURE_XG_NN_ALLNET.ps1';Set-Content $temp $src -Encoding utf8
$env:MZ_ALLNET_OUT=(Join-Path $OutDir 'memory')
$env:MZ_ALLNET_SCRIPT=(Join-Path $workspace 'tools\xg-nn-allnet-input-v1\decode_allnet.py')
$env:MZ_ALLNET_MODEL=$ModelPath
