...
if($row.analysis_found -and $row.mentions_1ply){
  $scanOut=Join-Path $env:MZ_STRUCTSCAN_OUT ([string]$c.case_id)
  New-Item -ItemType Directory -Force -Path $scanOut|Out-Null
  & python $env:MZ_STRUCTSCAN_SCRIPT ([string]$xg.Id) $env:MZ_STRUCTSCAN_MODEL $scanOut
  if($LASTEXITCODE -ne 0){throw "NN struct scanner exit $LASTEXITCODE"}
}
'@
$src=$src.Replace($needle,$inject)
$temp=Join-Path $env:RUNNER_TEMP 'CAPTURE_XG_NN_STRUCT_SCAN.ps1';Set-Content $temp $src -Encoding utf8
$env:MZ_STRUCTSCAN_OUT=(Join-Path $OutDir 'memory')
$env:MZ_STRUCTSCAN_SCRIPT=(Join-Path $workspace 'tools\xg-nn-struct-scan-v1\scan_struct.py')
$env:MZ_STRUCTSCAN_MODEL=$ModelPath
