...
if len(ref)<250:
    raise RuntimeError(f'reference must contain 250 inputs, got {len(ref)}')

def fbytes(vals): return b''.join(struct.pack('<f',float(x)) for x in vals)
patterns={
    'full250_v4':fbytes(ref[:250]),
    'base200_v4':fbytes(ref[:200]),
    'half100_v4':fbytes(ref[:100]),
    'engineered25_v4_a':fbytes(ref[200:225]),
    'engineered25_v4_b':fbytes(ref[225:250]),
    'engineered50_v4':fbytes(ref[200:250]),
}

# Starting-board occupancy alternatives, used only to isolate the base-channel
# convention.  The XG runtime result, not these hypotheses, is the oracle.
def make_half(kind):
    b=[0]*25
        ...
        while off<size:
            ...
            for name,pat in patterns.items():
                ...
                while True:
                    ...
                    if j<0: break
                    a=blob_base+j
                    hits.append({'pattern':name,'address':hex(a),'region_base':hex(base),'region_size':size})
                    pos=j+1
...
for i,r in enumerate(uniq):
    a=int(r['address'],16)
    start=max(0,a-1024); data=read_mem(start,8192)
...
with open(os.path.join(out,'SUMMARY.txt'),'w',encoding='utf-8') as f:
    f.write('XG_POST_ANALYSIS_MEMORY_SCAN_V3\n')
